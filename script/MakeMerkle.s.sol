...
contract MakeMerkle is Script, ScriptHelper {
    ...
    string private inputPath = "/script/json/input.json";
    string private outputPath = "/script/json/output.json";
    ...
    // This is because we will be storing the leaf nodes, inputs, and outputs for each leaf node
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            for (uint256 j = 0; j < types.length; ++j) {
                if (compareStrings(types[j], "address")) {
                    ...
                    uint256 value = vm.parseUint(elements.readString(getValuesByIndex(i, j))); // The purpose of parsing is to handle large uint256 values correctly from the JSON string
            ...
            // is declared in memory, memory arrays have a 32 byte offset and a 32 byte length prefix.
        ...
        for (uint256 i = 0; i < count; ++i) { // This part generates the proofs and output json entries
